...
const VideoPlaceholder = () => {
  ...
  return (
    ...
    >
      {/* HLS video element is appended here by the context (always alive) */}
      ...
      {!liveFrame && isSyncing && !isLoading && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/40 z-10 backdrop-blur-[1px]">
          <div className="bg-black/60 px-4 py-2 rounded-lg text-white flex items-center gap-3">
            ...
            <span className="text-sm font-medium">Syncing live feed...</span>
          </div>
        </div>
      )}

      {!liveFrame && error && !isLoading && !isSyncing && (
        <div className="absolute inset-0 flex items-center justify-center bg-black z-10">
